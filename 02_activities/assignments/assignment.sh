#!/bin/bash
set -x
mkdir analysis output
touch README.md
touch analysis/main.py
curl -Lo rawdata.zip https://github.com/UofT-DSI/shell/raw/refs/heads/main/02_activities/assignments/rawdata.zip
unzip rawdata.zip
mkdir data
mv rawdata ./data/raw
ls ./data/raw
mkdir -p ./data/processed/server_logs ./data/processed/user_logs ./data/processed/event_logs
#!/bin/bash
set -x

############################################
# DSI CONSULTING INC. Project setup script #
############################################
# This script creates standard analysis and output directories
# for a new project. It also creates a README file with the
# project name and a brief description of the project.
# Then it unzips the raw data provided by the client.

mkdir analysis output
touch README.md
touch analysis/main.py

# download client data
curl -Lo rawdata.zip https://github.com/UofT-DSI/shell/raw/refs/heads/main/02_activities/assignments/rawdata.zip
unzip rawdata.zip

###########################################
# Complete assignment here

# 1. Create a directory named data
mkdir data

# 2. Move the ./rawdata directory to ./data/raw
mv rawdata ./data/raw

# 3. List the contents of the ./data/raw directory
ls ./data/raw

# 4. In ./data/processed, create the following directories: server_logs, user_logs, and event_logs
mkdir -p ./data/processed/server_logs ./data/processed/user_logs ./data/processed/event_logs

# 5. Copy all server log files (files with "server" in the name AND a .log extension) from ./data/raw to ./data/processed/server_logs
find ./data/raw -type f -iname "*server*.log" -exec cp {} ./data/processed/server_logs/ \;

# 6. Repeat the above step for user logs and event logs
find ./data/raw -type f -iname "*user*.log" -exec cp {} ./data/processed/user_logs/ \;
find ./data/raw -type f -iname "*event*.log" -exec cp {} ./data/processed/event_logs/ \;

# 7. For user privacy, remove all files containing IP addresses (files with "ipaddr" in the filename) from ./data/raw and ./data/processed/user_logs
find ./data/raw ./data/processed/user_logs -type f -iname "*ipaddr*" -exec rm {} \;

# 8. Create a file named ./data/inventory.txt that lists all the files in the subfolders of ./data/processed
find ./data/processed -type f > ./data/inventory.txt

###########################################

echo "Project setup is complete!"

############################################
# DSI CONSULTING INC. Project setup script #
############################################
# This script creates standard analysis and output directories
# for a new project. It also creates a README file with the
# project name and a brief description of the project.
# Then it unzips the raw data provided by the client.

mkdir analysis output
touch README.md
touch analysis/main.py

# download client data
curl -Lo rawdata.zip https://github.com/UofT-DSI/shell/raw/refs/heads/main/02_activities/assignments/rawdata.zip
unzip rawdata.zip

###########################################
# Complete assignment here

# 1. Create a directory named data
mkdir data

# 2. Move the ./rawdata directory to ./data/raw
move .\rawdata .\data\raw


# 3. List the contents of the ./data/raw directory
dir .\data\raw


# 4. In ./data/processed, create the following directories: server_logs, user_logs, and event_logs
cd .\data\proceed
mkdir server_logs user_logs event_logs

# 5. Copy all server log files (files with "server" in the name AND a .log extension) from ./data/raw to ./data/processed/server_logs
mkdir -p ./data/processed/server_logs
cp ./data/raw/*server*.log ./data/processed/server_logs/


# 6. Repeat the above step for user logs and event logs
# Create target directories
mkdir -p ./data/processed/server_logs
mkdir -p ./data/processed/user_logs
mkdir -p ./data/processed/event_logs

# Copy server log files
cp ./data/raw/*server*.log ./data/processed/server_logs/

# Copy user log files
cp ./data/raw/*user*.log ./data/processed/user_logs/

# Copy event log files
cp ./data/raw/*event*.log ./data/processed/event_logs/
 

# 7. For user privacy, remove all files containing IP addresses (files with "ipaddr" in the filename) from ./data/raw and ./data/processed/user_logs
# Remove files with "ipaddr" in the name from ./data/raw
find ./data/raw -type f -name "*ipaddr*" -exec rm -f {} \;

# Remove files with "ipaddr" in the name from ./data/processed/user_logs
find ./data/processed/user_logs -type f -name "*ipaddr*" -exec rm -f {} \;

# 8. Create a file named ./data/inventory.txt that lists all the files in the subfolders of ./data/processed
find ./data/processed -type f > ./data/inventory.txt
cat ./data/inventory.txt



###########################################

echo "Project setup is complete!"
# Create directories for logs
mkdir -p ./data/processed/server_logs
mkdir -p ./data/processed/user_logs
mkdir -p ./data/processed/event_logs

# Copy log files to their respective directories
cp ./data/raw/*server*.log ./data/processed/server_logs/
cp ./data/raw/*user*.log ./data/processed/user_logs/
cp ./data/raw/*event*.log ./data/processed/event_logs/

# Remove files containing "ipaddr" for privacy
find ./data/raw -type f -name "*ipaddr*" -exec rm -f {} \;
find ./data/processed/user_logs -type f -name "*ipaddr*" -exec rm -f {} \;

# Create an inventory file listing all files in ./data/processed
find ./data/processed -type f > ./data/inventory.txt

# Print completion message
echo "Project setup is complete!"



